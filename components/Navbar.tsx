...
function NavItem({ title, subItems }: NavItemProps) {
  ...
  return (
    ...
    >
      {/* Main Menu Item */}
      <div className="flex items-center justify-between w-full md:w-auto py-2 md:py-0   text-left md:text-center text-TextColor text-lg font-medium">
        {title}
      ...
      >
        {subItems.map((item) => (
          <li key={item.title}>
            <a href={item.href} className="block p-2 hover:bg-gray-100">
              {item.title}
